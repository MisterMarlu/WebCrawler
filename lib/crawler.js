...
Crawler.prototype.crawl = async function (callback) {
  ...
  if (this.numVisited >= this.pageLimit && this.pageLimit !== 0) return 'page limit';
...
Crawler.prototype.visitPage = async function (url, callback) {
  ...
  await axios.get(url)
    .then(async function (response) {
      ...
      if (response.status !== 200) return self.crawl(callback);

      let $ = cheerio.load(response.data);

      // Call the callback function so the customer can do everything he want.
      if (typeof callback === 'function') await callback($, url, self);
...
Crawler.prototype.collectRelativeLinks = function (link) {
  ...
  if (link.length > 0) {
    if (this.linkList[linkType].indexOf(tmpUrl) === -1) {
      ...
      if (linkType === 'typeRelative') this.followingPages.push(tmpUrl);
    }
  }
...
Crawler.prototype.isDefault = function (command) {
  if (!this.commands.hasOwnProperty(command)) return false;
