...
Crawler.prototype.collectLinks = async function ($) {
  ...
  await links.each(function () {
    ...
    if (self.collectRelativeLinks(href)) {
      self.DB.findOne({url: self.startUrl}, {}, 'starting_urls', function (error, portal) {
        if (error) throw error;

        let oldObject = {starting_urls_id: portal._id.toString(), url: href};
