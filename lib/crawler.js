...
Crawler.prototype.collectLinks = async function ($) {
  this.output.writeConsole(`Crawler.collectLinks - File: ${__filename} - Line: ${__line}`, isDebug(Global.get('DEBUG'), moduleName), 'debug');
  let self = this,
    links = $("a[href^='/']"),
    abLinks = $("a[href^='http']"),
    count = 0,
...
Crawler.prototype.collectRelativeLinks = function (link) {
  this.output.writeConsole(`Crawler.collectRelativeLinks - File: ${__filename} - Line: ${__line}`, isDebug(Global.get('DEBUG'), moduleName), 'debug');
