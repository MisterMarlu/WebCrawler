...
class Crawler extends BaseModule {
  ...
  async crawlViaObjects() {
    // Debugging line.
    this.output.writeConsole(`${moduleName}.crawlViaObjects - File: ${__filename} - Line: ${__line}`, Helper.isDebug(moduleName), 'debug');
  ...
  async crawlViaDb() {
    // Debugging line.
    this.output.writeConsole(`${moduleName}.crawlViaDb - File: ${__filename} - Line: ${__line}`, Helper.isDebug(moduleName), 'debug');
    ...
    this.db.findOne({}, 'found_urls').then(page => {
      ...
      // Remove the url, which we want to visit, from the database.
      this.db.delete(page, 'found_urls').then(result => {
   ...
   *
   * @param {object} $ The content of the website we can perform like in jQuery.
   * @param {boolean} [db=false] Using the database variant.
   * @returns {Promise} Returns an empty Promise object.
   */
