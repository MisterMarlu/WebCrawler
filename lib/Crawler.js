...
class Crawler extends BaseModule {
  ...
  async start(stop) {
    // Debugging line.
    Helper.printDebugLine(instance.output, this.start, __filename, __line);
    ...
    this.createLockFile();
    this.startTime = new Date();
    this.crawlViaObjects();
  }

  /**
   * Write ending lines into the .log file and remove .lock file.
   */
  end() {
    // Debugging line.
    Helper.printDebugLine(instance.output, instance.end, __filename, __line);
    ...
    instance.clearCacheDB().then(() => {
      // Remove the information with the spinner.
      tmpSpinner.stop(true);

      // Write some ending lines.
      if (Global.get('useDB')) {
        instance.output.writeLine(`Used database until page ${Global.get('useDB')}`, true);
      }

      instance.output.writeUserInput(instance);
      instance.getReadableTime();

      // Close the write stream to the log file.
      instance.output.logger.end();
  ...
  clearCacheDB() {
    // Debugging line.
    Helper.printDebugLine(instance.output, this.clearCacheDB, __filename, __line);
  ...
  async crawlViaObjects() {
    // Debugging line.
    Helper.printDebugLine(instance.output, this.crawlViaObjects, __filename, __line);

    // When we visited 1,000 urls check the speed of the database.
    if (this.numVisited === 1000) {
      // Call the equivalent method with database connection once.
      this.crawlViaDb();
      return;
    }
    ...
    if (typeof nextPage === 'undefined') return this.stop('last page');
    ...
    this.db.delete({url: nextPage}, 'found_urls').then(result => {
      ...
      if (this.visited.includes(nextPage)) return this.crawlViaObjects();
      ...
      if (urlTime > (this.foundUrlTime * 20)) {
        ...
        // Sync the current found urls with the database.
  ...
  async crawlViaDb() {
    // Debugging line.
    Helper.printDebugLine(instance.output, this.crawlViaDb, __filename, __line);
      ...
      this.db.delete({url: page.url}, 'found_urls').then(result => {
        this.db.findOne({url: nextPage}, 'visited_urls').then(result => {
          ...
          if (this.checkUrlTime) {
            ...
            console.log(this.foundUrlTime);
            console.log(this.foundUrlTime * 20);
            useDB = false;
          }
  ...
  async visitPage(url, db = false) {
    // Debugging line.
    Helper.printDebugLine(instance.output, this.visitPage, __filename, __line);
  ...
  increaseVisited(url) {
    // Debugging line.
    Helper.printDebugLine(instance.output, this.increaseVisited, __filename, __line);
  ...
  async collectLinks($, db = false) {
    // Debugging line.
    Helper.printDebugLine(instance.output, this.collectLinks, __filename, __line);
  ...
  collectRelativeLinks(link, db = false) {
    // Debugging line.
    Helper.printDebugLine(instance.output, this.collectRelativeLinks, __filename, __line);
  ...
  isDefault(command) {
    // Debugging line.
    Helper.printDebugLine(instance.output, this.isDefault, __filename, __line);
  ...
  getReadableTime() {
    // Debugging line.
    Helper.printDebugLine(instance.output, this.getReadableTime, __filename, __line);
  ...
  createLockFile() {
    // Debugging line.
    Helper.printDebugLine(instance.output, this.createLockFile, __filename, __line);
  ...
  removeLockFile() {
    // Debugging line.
    Helper.printDebugLine(instance.output, this.removeLockFile, __filename, __line);
  ...
  hasLockFile() {
    // Debugging line.
    Helper.printDebugLine(instance.output, this.hasLockFile, __filename, __line);
  ...
  syncFoundUrls(i = 0) {
    // Debugging line.
    Helper.printDebugLine(instance.output, this.syncFoundUrls, __filename, __line);
  ...
  static checkSaveTimer() {
    // Debugging line.
    Helper.printDebugLine(instance.output, Crawler.checkSaveTimer, __filename, __line);
