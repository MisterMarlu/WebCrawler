...
const spinnerText = '%s Waiting for screenshots. ',
  config = require('../config.json').screenshot;
...
Screenshot.prototype.screenshotting = async function (website, debug) {
  ...
  let name = this.getScreenshotName(website.website, website.imprint),
    ...
    popup = false,
    screenshotOptions = {
      filePath: config.screenshot.filePath + name + '.png'
    };
  ...
  try {
    entry = await chromeless
      .goto(url)
      .wait(config.wait.entry.before)
      .exists(config.exists.entry);
  ...
  try {
    popup = await chromeless
      .goto(url)
      .wait(config.wait.popup.before)
      .exists(config.exists.popup);
  ...
  if (entry && !popup) {
    try {
      screenshot = await chromeless
        .goto(url)
        .wait(config.wait.entry.before)
        .click(config.exists.entry)
        .wait(config.wait.entry.after)
        .setViewport(config.viewPort)
        .wait(config.wait.default)
        .screenshot(screenshotOptions);
    } catch (error) {
  ...
  if (!entry && popup) {
    try {
      screenshot = await chromeless
        .goto(url)
        .wait(config.wait.popup.before)
        .click(config.exists.popup)
        .wait(config.wait.popup.after)
        .setViewport(config.viewPort)
        .screenshot(screenshotOptions);
    } catch (error) {
