...
ScreenShot.prototype.doScreenshots = async function (websites, debug, callback) {
  ...
  exec('google-chrome-stable --headless --hide-scrollbars --remote-debugging-port=9222 --disable-gpu', function (error, stdout, stderr) {
  ...
  for (let i = 0; i < websites.length; i += 1) {
    if (!websites[i].has_error) {
      ...
      tmpSpinner.start();
      // Do the screenshot.
      await this.screenshotting(websites[i], debug, callback);
      tmpSpinner.stop(true);
    }
  }

  exec('killall google-chrome-stable', function (error, stdout, stderr) {
    if (error) throw error;
  });
 ...
 */
ScreenShot.prototype.screenshotting = async function (website, debug, callback) {
  ...
  this.DB.save(oldObject, newObject, 'screenshots', function (error, result) {
    if (error) throw error;
  });
