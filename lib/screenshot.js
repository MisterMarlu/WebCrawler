...
const {parseTime} = require(`${__dirname}/parser`);
 ...
 *
 * @param websites: {[{url: string, hasError: boolean, pageName: string}]}
 * @param debug: {boolean}
 * @returns {string}
 */
 ...
 *
 * @param website: {{url: string, name: string, hasError: boolean}}
 * @param debug: {boolean}
...
ScreenShot.prototype.screenshotting = async function (website, debug) {
  ...
  output.write(`Added screenshot (${this.done}) ${screenshot}`);
  this.decreaseUndone();
};
