...
ScreenShot.prototype.doScreenshots = async function (websites, debug) {
  ...
  return {
    ...
    screenshotsDone: this.screenshotsDone
  };
};
...
ScreenShot.prototype.screenshotting = async function (website, debug) {
  ...
  try {
    ...
    this.output.writeLine(`Check for entry button on "${url}". Entry button: "${entry}"`, debug, 'debug');
  } catch (error) {
  ...
  try {
    ...
    this.output.writeLine(`Check for popup on "${url}". Popup: "${popup}"`, debug, 'debug');
  } catch (error) {
  ...
  let oldObject = {url: website.found_url, file_path: screenshot},
    structure = {_id: false, updated: false},
    newObject = {url: website.found_url, file_path: screenshot, updated: new Date()};
