 ...
 * @constructor
 */
let Output = function (projectPath) {
  if (!(this instanceof Output)) return new Output(projectPath);
 ...
 *
 * @param sentences: {[{text: string, color: string, bg: string}]}
 * @param type?: {string}
 */
...
Output.prototype.initLogger = function (name) {
  ...
  let path = this.logPath || `${this.projectPath}/logs`;

  if (!fs.existsSync(path)) fs.mkdirSync(path);

  path += `/${name}.log`;
  let info = checkLogFile(name, path, this.multiple);
 ...
 */
function checkLogFile(name, path, multiple) {
