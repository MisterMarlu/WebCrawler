 ...
 * @param background?: {string}
 */
Output.prototype.writeConsole = function (value, type, background) {
  if (typeof type === 'undefined' || type.length < 1) {
    console.log(value);
    return;
  }
...
Output.prototype.writeUserInput = function (crawler) {
  this.writeLine('User input:', true, 'underscore');
  ...
  for (let name in crawler.commands) {
    if (crawler.commands.hasOwnProperty(name)) {
      ...
      this.write(`${name}: ${crawler.commands[name]}${string}`, true, color);
    }
  }
};
