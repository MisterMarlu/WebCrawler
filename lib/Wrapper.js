 ...
 * @param {string} [options.debugLevel=0] The level to debug, from 0 to 5 where 5 is the highest level.
...
class Wrapper extends BaseModule {
  ...
  constructor(dir, commands) {
    ...
    // saveTimer is the timer that tells DelayedSave when to save the cached data.
    ...
    // connString is the connection string to a MongoDB database.
    this.connString = '';
    // connection stores the connection information for a MySQL database.
    this.connection = {};
    this.dbVariant = '';
   ...
   * INFO: I know that this method can be static, but I don't want to make it static.
   *
  ...
  performCustomModules() {
    ...
    for (let i = 0; i < customerModules.length; i += 1) {
      ...
      let newModule = require(`${this.projectPath}${moduleInfo.pathToModule}`),
        // Default options.
        options = {
  ...
  setCallback(name, callback) {
    // Debugging line.
    Helper.printDebugLine(this.setCallback, __filename, __line);
  ...
  addConfig(pathToConfig, name) {
    // Debugging line.
    Helper.printDebugLine(this.addConfig, __filename, __line);
  ...
  addModule(pathToModule) {
    // Debugging line.
    Helper.printDebugLine(this.addModule, __filename, __line);
  ...
  /**
   * Search for a default configuration file in the project root called "web-crawler.json".
   */
  searchForConfig() {
  ...
  /**
   * Check if a database connection string or object is defined.
   */
  databaseCheck() {
    // Debugging line.
    Helper.printDebugLine(this.databaseCheck, __filename, __line);
  ...
  saveStartingUrl(url) {
    // Debugging line.
    Helper.printDebugLine(this.saveStartingUrl, __filename, __line);
   ...
   *
   * @param {string} logFileName Name of the log file. Default is the name defined in the config.json.
   */
  startWithMongo(logFileName) {
    // Debugging line.
    Helper.printDebugLine(this.startWithMongo, __filename, __line);
    ...
    MongoClient.connect(this.connString, (error, db) => {
      ...
      this.saveStartingUrl(Global.get('START_URL')).then(() => {
        // Stop and remove the spinner from the console.
  ...
  startWithMysql(logFileName) {
    // Debugging line.
    Helper.printDebugLine(this.startWithMysql, __filename, __line);
  ...
  crawl(logFileName = '') {
    // Debugging line.
    Helper.printDebugLine(this.crawl, __filename, __line);
  ...
  startCrawling() {
    // Debugging line.
    Helper.printDebugLine(this.startCrawling, __filename, __line);
  ...
  static stopCrawling(reason) {
    // Debugging line.
    Helper.printDebugLine(Wrapper.stopCrawling, __filename, __line);

    // Do not try to start a saving process when already saving.
    ...
    instance.delayedSave.saveAll().then(() => {
      // If screenshots are enabled, call the callback to create some.
      if (Global.get('SCREEN_SHOTS')) {
        if (Helper.hasCallback('screenshot')) {
          // Tell the user that we are creating screenshots.
  ...
  static ending(reason) {
    // Debugging line.
    Helper.printDebugLine(Wrapper.ending, __filename, __line);
    ...
    // Stop all processes, like the open stream to write into the log file.
