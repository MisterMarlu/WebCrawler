...
/**
 * Get customer id from Ladies.de.
 *
 * @param $
 * @returns {int}
 */
exports.getCustomerId = function ($) {
  let src = $('.ladies-tracking').attr('src');

  if (typeof src === 'undefined' || src.length === 0) return 0;

  let srcInfo = new URL(src),
    query = queryString.parse(srcInfo.query);

  return (query.place / 33);
};